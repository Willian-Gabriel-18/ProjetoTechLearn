export default defineEventHandler(async (event) => {
  ...
  const [trilhas, porNivelRows, feitasPorUsuario, fila] = await Promise.all([
    sql`
    ...
    `,
    sql`
      SELECT a.trilha_id, a.nivel, count(*)::int AS total
      FROM aulas a
      WHERE a.publicada = true
      GROUP BY a.trilha_id, a.nivel
    `,
    usuario
      ? sql`
    ...
    usuario
      ? sql`
          SELECT a.id, a.slug, a.titulo, a.trilha_id, a.ordem, a.nivel
          FROM aulas a
  ...
  for (const t of trilhas) {
    ...
    for (const row of porNivelRows) {
      if (row.trilha_id !== t.id) continue
