...
<template>
  <RouterLink :to="destinationUrl" v-if="renderizar" class="demarcar md:w-30p py-7 px-5 h-full rounded-md flex flex-col flex-nowrap md:hover:scale-105 md:hover:cursor-pointer">
    ...
    <div class="mt-2">
      <h4 class="text-3xl text-stone-700 uppercase font-bold overflow-hidden overflow-ellipsis" :title="title">{{ title }}</h4>
      <p class="text-pretty text-justify text-xl text-stone-700 overflow-hidden">{{ resumoFormatado(resume, 100) }}</p>
    </div>

    <div class="flex flex-nowrap flex-row justify-between items-center mt-4">
      <div v-if="listIconClass" class="max-w-50p text-stone-800 text-2xl flex flex-row flex-nowrap gap-2 items-center overflow-hidden mr-auto">
        <i v-for="(item, index) in listIconClass.slice(0, 5 || listIconClass.length)" :key="index" :class="'pi ' + item"> </i>
      </div>
