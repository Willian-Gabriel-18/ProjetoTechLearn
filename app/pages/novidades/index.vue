...
<template>
  <section class="mx-auto max-w-5xl px-4 py-10">
    ...
    <p class="mt-3 text-lg">Dicas e avisos da plataforma — fora da trilha, para não misturar com a aula.</p>
    <ul v-if="data?.noticias?.length" class="mt-8 space-y-4">
      <li v-for="n in data.noticias" :key="n.id" class="border border-linha rounded-md p-4">
        <NuxtLink :to="`/novidades/${n.slug}`" class="font-display text-2xl underline text-cerrado">
          {{ n.titulo }}
        </NuxtLink>
